...
class ListDataDetailRepository extends RestApiService {
  ...
  void createFileRecursively(String folder, Uint8List response) async {
    // Get the image
    ...
    Directory writeDir = Directory.fromUri(
        Uri(path: '$extDir/$folder')); // Join the folder and external storage
    writeDir.createSync(
        recursive:
            true); // We first create the folder and file if it does not exist.

    String filename = 'myfile${DateTime.now().millisecondsSinceEpoch}.xlsx';

    File('$extDir/$folder/$filename.jpeg').writeAsBytes(response,
        flush:
...
//=======================
  Future<File> writeToFile(Uint8List data) async {
    ...
    var filePath =
        '$tempPath/file_01${DateTime.now().millisecondsSinceEpoch}.csv'; // file_01.tmp is dump file, can be anything
    return File(filePath).writeAsBytes(
        buffer.asUint8List(data.offsetInBytes, data.lengthInBytes));
  }
//======================
...
//=======================
  Future<File> writeToFile1(Uint8List data) async {
    ...
    String tempPath = tempDir.path;
    var filePath =
        '$tempPath/file_01${DateTime.now().millisecondsSinceEpoch}.xlsx'; // file_01.tmp is dump file, can be anything
