...
class MyPendingApprovalBloc
    extends Bloc<MyPendingApprovalEvent, MyPendingApprovalState> {
  ...
  Future<void> _onMyPendingApprovalStarted(MyPendingApprovalStarted event,
      Emitter<MyPendingApprovalState> emit) async {
    try {
      emit(state.copyWith(status: BaseStatusEnum.inprogress));

      final userId = Application.authBloc.state.userToken?.user.id;
      final isProvincialHead = Application
              .authBloc.state.userToken?.user.isProvincialDepartmentHead ??
          false;
      final isDataVerifier1 =
          Application.authBloc.state.userToken?.user.isDataVerifier1 ?? false;
      final isDataVerifier2 =
          Application.authBloc.state.userToken?.user.isDataVerifier2 ?? false;
      final isDepartmentHead =
          Application.authBloc.state.userToken?.user.isHeadDepartment ?? false;

      List<PendingApprovalModel> myPendingApproval = [];

      if (isProvincialHead) {
        final provincialDepartmentHeadProvinceId = Application.authBloc.state
                .userToken?.user.provincialDepartmentHeadProvinceId ??
            0;
        myPendingApproval =
            await repository.getMyPendingApprovalListProvincialHead(
        ...
        myPendingApproval =
            await repository.getMyPendingApprovalListVerifier2();
      } else if (isDepartmentHead) {
        myPendingApproval =
            await repository.getMyPendingApprovalListDepartmentHead();
      } else {
        myPendingApproval =
            await repository.getMyPendingApprovalList(userId ?? 0);
      }
